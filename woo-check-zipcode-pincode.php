...
if (!class_exists('WCZP')) {
    ...
    class WCZP {
        ...
        function WCZP_show_notice() {
       	...
       	function WCZP_plugin_row_meta( $links, $file ) {
            if ( WCZP_BASE_NAME === $file ) {
                $row_meta = array(
                    'rating'    =>  '<a href="#" target="_blank"><img src="'.WCZP_PLUGIN_DIR.'/includes/images/star.png" class="wczp_rating_div"></a>',
                );
        ...
        function WCZP_load_front() {
            ...
            wp_localize_script( 'WCZP_front_script', 'ajax_postajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
            $translation_array = WCZP_PLUGIN_DIR;
            wp_localize_script( 'WCZP_front_script', 'object_name', $translation_array );
        }
